    ...
    var easyModule = {
      ...
      getCurrentScript: function () {
        ...
        if (stack) {
          ...
          // 去掉换行符
          stack =
            stack[0] === "(" ? stack.slice(1, -1) : stack.replace(/\s/, "");
          //去掉行号与或许存在的出错字符起始位置
      ...
      create: function (url, name, useKey) {
        ...
        if (isScriptOnload) {
          script.onerror = function () {
            ...
            easyModule.error(
              "[" + name + "] module failed to load, the url is " + url + "."
            );
          };
        }
      ...
      config: function (options) {
        ...
        if (options.baseUrl) {
          ...
          // 相对路径的baseUlr是基于HTML页面所在的路径(无论是http地址还是file地址)
            ...
            moduleOptions.baseUrl = easyModule.mergePath(
              baseUrl,
